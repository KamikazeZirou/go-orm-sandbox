...
var (
	labelAllColumns            = []string{"id", "name"}
...
func (o *Label) Issues(mods ...qm.QueryMod) issueQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`issue_labels` on `issues`.`id` = `issue_labels`.`issue_id`"),
		qm.Where("`issue_labels`.`label_id`=?", o.ID),
	)
...
func (labelL) LoadIssues(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLabel interface{}, mods queries.Applicator) error {
	var slice []*Label
	var object *Label
	...
	query := NewQuery(
		qm.Select("`issues`.id, `issues`.title, `issues`.description, `issues`.bad_col_name, `issues`.private_col_name, `issues`.updated_at, `issues`.created_at, `a`.`label_id`"),
		qm.From("`issues`"),
		qm.InnerJoin("`issue_labels` as `a` on `issues`.`id` = `a`.`issue_id`"),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Title, &one.Description, &one.GoodColName, &one.PrivateColName, &one.UpdatedAt, &one.CreatedAt, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for issues")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice issues")
		}
...
func (o *Label) AddIssues(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Issue) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into `issue_labels` (`label_id`, `issue_id`) values (?, ?)"
...
// SetIssues removes all previously related items of the
// label replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Labels's Issues accordingly.
// Replaces o.R.Issues with related.
// Sets related.R.Labels's Issues accordingly.
func (o *Label) SetIssues(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Issue) error {
	query := "delete from `issue_labels` where `label_id` = ?"
	...
	removeIssuesFromLabelsSlice(o, related)
	if o.R != nil {
		o.R.Issues = nil
	}
	return o.AddIssues(ctx, exec, insert, related...)
}

// RemoveIssues relationships from objects passed in.
// Removes related items from R.Issues (uses pointer comparison, removal does not keep order)
// Sets related.R.Labels.
func (o *Label) RemoveIssues(ctx context.Context, exec boil.ContextExecutor, related ...*Issue) error {
	var err error
	query := fmt.Sprintf(
		"delete from `issue_labels` where `label_id` = ? and `issue_id` in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.Issues {
			...
			ln := len(o.R.Issues)
			if ln > 1 && i < ln-1 {
				o.R.Issues[i] = o.R.Issues[ln-1]
			}
			o.R.Issues = o.R.Issues[:ln-1]
			break
		}
	}
...
func removeIssuesFromLabelsSlice(o *Label, related []*Issue) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
